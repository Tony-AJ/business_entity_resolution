...
rose from 0.954 to 0.966; final v110: **TBD**.
...
- **Candidate pairs generated:** v107: 60.52M blocked test pairs (34.9 per S1), 4.8–6.0 per
  S1 after the filter; v110: **TBD** (69.3 per S1 blocked at mock density; **TBD** after the
  filter).
- **How you ensured true matches were not lost:** address passes catch pairs sharing no name
  ...
  `candidate_pairs.tsv`: 34.5 → 4.6 per S1 for −0.0011 recall (v104; v110: **TBD**).
...
- Other: blocking cosines, IDF-weighted overlaps, rank and gap to the entity's best candidate,
  ...
  0.9870 (v040). Stage 2 adds 20 from the stage-1 probabilities p1: competition (rank, best
  rival, gap), anchor (vs the entity's best other candidate) and rival (vs the record's best
  rival S1).

**Model type:** stage 1: XGBoost on the GPU (127 leaves) on 110k train-fold entities absent
from the mock (~9M pairs); LightGBM up to v107. Stage 2: XGBoost on the GPU (63 leaves, early
stopping on mock tune entities) on the mock's fit entities, cross-fitted in two parts;
`pool_gap` (p1 minus the record's best rival) carried 72 % of its gain in v104.
...
| **v110** | + 23 features, denser blocking, GPU stage 1, rival features | **TBD** | **TBD** | **TBD** |
...
- **Common false positives (wrong merges):** same-name decoys the address cannot separate: an
  ...
  (v107), **TBD** (v110).
- **Common false negatives (missed matches):** misses rose in exchange (64k → 74k pairs; v110
  **TBD**): Indic-script names (`Baba Food` ↔ बाबा फूड), domain forms, truncations, renames,
...
make nb NB=experiments/v110_m3_features/v110_m3_features.ipynb   # writes output/*.tsv
make validate
```
...
from XGBoost 3.1.1 (Apache-2.0) and, up to v107, LightGBM 4.7.0 (MIT); v110 has ≤ 1.8M
parameters, far below 8B; no GPL package. Country is only a partition key (no filter, feature
or threshold): France, unseen in train, gets a row per entity (v107: 94.5 % of French S1
matched; India 93.7 %, US 94.1 %).

### B. Additional Results

Mock F0.5 India / US: v107 0.9701 / 0.9792, v110 **TBD** / **TBD**.
v110 est_public: stage 1 alone **TBD**; stage 2 without the 23 new features **TBD**, without
rival features **TBD**.
