...
_COMPUTE_ORDER = ("blocking", "legal", "numeric", "address", "context", "name_fuzzy",
                  "name_tokens", "meta", "pool_context", "phonetic")
...
# phonetic is opt-in too, but for a different reason: it is a new feature group (needs the
# jellyfish/metaphone dependencies) awaiting a local-F0.5 shortlist decision (project rule 3)
# before it can join the default set; enable it explicitly via ``feature_groups=``.
DEFAULT_GROUPS: tuple[str, ...] = tuple(g for g in REGISTRY if g not in ("pool_context",
                                                                        "phonetic"))
