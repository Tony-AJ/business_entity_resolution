...
Last updated: 2026-09-26 09:55 IST (day 2).
...
| 26 | Mock-test fold + global 1-to-1 scoring, tuning on mock tune S1, scoring on mock val S1; tests | M1 | INT / E2 | 11:30 | todo | `split` / `evaluate` / `pipeline` |
| 27 | Mock blocking cache: ~1.37M S1 × ~8.0M pool, per country | M1 | A | 12:30 | todo | ~45 min in the background |
...
| 29 | v103: v101 matcher + rule tuned on the mock tune S1; test inference; **upload #3** | M1 | E2 | 14:15 | todo | Upload only if mock F0.5 ≥ v101 + 0.003 |
...
| 33 | v105: two-stage matcher: stage-1 out-of-fold probs → S1-side and pool-side competition features (rank, gap, best rival, rivals ≥ 0.5) → stage-2 LightGBM | M1 | C5 / D3 | 19:00 | todo | Was #17 (pool-side competition) |
| 34 | Stage-1 candidate filter: keep the top ~10 per S1 above a floor, so `candidate_pairs.tsv` shrinks from ~35 to ≤ 12 per S1 at ≤ 0.002 recall loss | M1 | A5 | 20:00 | todo | Was #13b (organisers' update: smaller candidate sets rank higher) |
| 35 | v106: best of v104–v105 + France + filter; test inference; **upload #4** | M1 | INT | 21:00 | todo | |
| 36 | LightGBM capacity (leaves 127/255, lr 0.03, more fit S1), expected-F0.5 set decoding, per-source thresholds | M1 | D3 / E5 | 23:00 | todo | Was #18, #21 |
