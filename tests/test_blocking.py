...
def test_topk_pass_chunking_invariant():
    """Querying the pool in two chunks gives the same pairs and sims as one piece (default)."""
    s1n, pooln = _typo_frames()
    full = block(s1n, pooln, _cfg())
    pd.testing.assert_frame_equal(full, block(s1n, pooln, _cfg(pool_chunk=len(pooln))))
    pd.testing.assert_frame_equal(full, block(s1n, pooln, _cfg(pool_chunk=len(pooln) // 2)))
...
def test_default_blocking_key_is_stable() -> None:
    ...
    assert replace(BlockingConfig(), pool_chunk=2_000_000).key() != "66540dae"
...
def test_union_single_pass_all_bits_and_unknown_pass():
    """A lone pass keeps its own bit and NaN sims elsewhere; every pass together ORs to
    the sum of the distinct bits; an unknown pass name is a programming error and raises
    ...
    assert union_passes(all_parts, 60)["pass"].tolist() == [sum(PASS_BITS.values())]
    assert sum(PASS_BITS.values()) == 255                  # eight distinct bits
